              ...
              <div className='relative'>
                ...
                <div className='border-2 border-dashed border-gray-500 dark:border-gray-500 p-3 bg-gradient-to-br from-green-50 to-slate-100 dark:from-gray-800 dark:to-gray-900 relative'
                  ...
                  {(() => {
                    ...
                    // Tarmac layout: continuous fence-to-fence trunks and connecting corridors.
                    if (!isMurramRoad) {
                      const tarmacTrunkList = []
                      if (!isColLayout) {
                        tarmacTrunkList.push({ dir: 'h', pos: ['top','top-left','top-right'].includes(gs) ? 'top' : 'bottom' })
                        if (gs === 'left') tarmacTrunkList.push({ dir: 'v', pos: 'left' })
                        if (gs === 'right') tarmacTrunkList.push({ dir: 'v', pos: 'right' })
                      } else {
                        tarmacTrunkList.push({ dir: 'v', pos: ['right','top-right','bottom-right'].includes(gs) ? 'right' : 'left' })
                        if (['top','top-left','top-right'].includes(gs)) tarmacTrunkList.push({ dir: 'h', pos: 'top' })
                        if (['bottom','bottom-left','bottom-right'].includes(gs)) tarmacTrunkList.push({ dir: 'h', pos: 'bottom' })
                      }

                      return (
                        <>
                          {tarmacTrunkList.map((t, i) => t.dir === 'h' ? (
                            <div key={i} className={`absolute left-0 right-0 overflow-hidden ${compoundRoadBgClass}`}
                              style={t.pos === 'top'
                            ...
                            <div key={i} className={`absolute top-0 bottom-0 overflow-hidden ${compoundRoadBgClass}`}
                              style={t.pos === 'left'
